...
class BlobReader(BlobMemoryReader):
    def __init__(self, client, buffer):
        super().__init__(client, len(buffer))
        self.buffer = buffer
...
class Blob(BlobOperationPut):
    ...
    def complete(self, reason, result):
        ...
        if reason == BlobReason.DONE:
            ...
            eprint(f"{self.instance_name}: EXIT")
...
def main():
    ...
    while True:
        try:
            ...
            eprint(f"{config.instance_name}: exiting the main loop ({e})")
            return -1
        ...
        if reported_step != step[0]:
            eprint(f"{config.instance_name}: Sending state {step[0]}")
            state = State(client)

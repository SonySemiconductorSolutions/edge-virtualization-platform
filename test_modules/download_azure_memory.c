...
static const char *module_name = "DOWNLOAD-AZURE-MEMORY";
...
blob_io_cb(void *buf, size_t buflen, void *userData)
{
	...
	log_module(module_vars->instance_name,
		   "Received--> buf: %p, buflen: %zu, offset: %d)\n", buf,
		   buflen, module_vars->blob_buff_offset);

	/* Check size requested by blob to ensure enough memory in local buffer
	 */
	assert((module_vars->blob_buff_offset + buflen) <=
	       module_vars->blob_buff_size);
	memcpy(module_vars->blob_buff + module_vars->blob_buff_offset, buf,
	       buflen);
...
blob_cb(EVP_BLOB_CALLBACK_REASON reason, const void *vp, void *userData)
{
	...
	case EVP_BLOB_CALLBACK_REASON_DONE:
		...
		log_module(module_vars->instance_name,
			   "Total blob size is %d.\n",
			   module_vars->blob_buff_offset);
...
main(void)
{
	...
	module_vars.blob_buff_size = BLOB_MEMORY_MAX_SIZE;
	module_vars.blob_buff_offset = 0;
	module_vars.blob_buff = malloc(module_vars.blob_buff_size);
	assert(module_vars.blob_buff != NULL);
	...
	log_module(module_vars.instance_name,
		   "Blob operations over memory are not supported in "
		   "Linux-based devices.\n"
		   "--- END TEST ---");
	assert(0);
#endif
	...
	for (;;) {
		...
		if (module_vars.filename != NULL &&
			...
			log_module(module_vars.instance_name,
				   "The blob will be downloaded to buffer "
				   "memory.\n"
				   "Note: A copy of blob will be stored to: "
				   "%s.\n",
		...
		if (module_vars.download != NULL &&
			...
			log_module(module_vars.instance_name,
				   "Scheduling a download to memory.\n");

			struct EVP_BlobRequestAzureBlob request;
			request.url = cb_data.blob_url = module_vars.download;
			module_vars.download = NULL;
			module_vars.localStore.filename = NULL;
			module_vars.localStore.io_cb = blob_io_cb;

			result = EVP_blobOperation(h, EVP_BLOB_TYPE_AZURE_BLOB,
		...
		if (module_vars.step == STEP_END_TEST) {
			log_module(module_vars.instance_name,
				   "Dump blob from memory to file.\n");
			FILE *fp = fopen(module_vars.abs_filename, "w");
			assert(fp != NULL);
			size_t nobj =
				fwrite(module_vars.blob_buff,
				       module_vars.blob_buff_offset, 1, fp);
			assert(nobj == 1);
			int ret = fclose(fp);
			assert(ret == 0);

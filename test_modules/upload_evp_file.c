...
main(void)
{
	...
	for (;;) {
		...
		if (module_vars.upload != NULL &&
			...
			/* Special case to check legacy STP HUB API. To force
			 * to send the old HUB API the module create a
			 * EVP_BLOB_TYPE_EVP_EXT request with storageName null
			 */
			if (strcmp(request.storage_name, "NULL") == 0) {

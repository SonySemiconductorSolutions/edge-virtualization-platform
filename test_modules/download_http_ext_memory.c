...
static const char *module_name = "DOWNLOAD-HTTP-EXT-MEMORY";
...
{
	assert(userData != NULL);
	const struct EVP_BlobResultHttpExt *result;
...
{
	/* We only care about a successful HTTP resonse.
...
main(void)
{
	...
	for (;;) {
		...
		if (result == EVP_SHOULDEXIT) {
			log_module(module_name, "MODULE GOT EVP_SHOULDEXIT\n");
			/* free buffers */

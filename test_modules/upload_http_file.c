...
static const char *module_name = "UPLOAD-HTTP-FILE";
...
blob_cb(EVP_BLOB_CALLBACK_REASON reason, const void *vp, void *userData)
{
	...
	case EVP_BLOB_CALLBACK_REASON_DONE:
		{
			const struct timespec *before = &module_vars->before;
			struct timespec now;

			if (clock_gettime(CLOCK_MONOTONIC, &now)) {
				log_module(module_vars->instance_name,
					   "clock_gettime(2) failed with "
					   "errno %d\n",
					   errno);
				assert(0);
			}

			intmax_t sec = now.tv_sec - before->tv_sec,
				 nsec = now.tv_nsec - before->tv_nsec;

			struct timespec diff = {
				.tv_sec = nsec >= 0 ? sec : sec - 1,
				.tv_nsec =
					nsec >= 0 ? nsec : 1000000000 + nsec,
			};
			...
			log_module(module_vars->instance_name,
				   ...
				   result->result, result->http_status,
				   result->error);

			log_module(module_vars->instance_name,
				   "<ci/blob/upload/time>: %s %ju.%09ju\n",
				   cb_data->blob_url, (uintmax_t)diff.tv_sec,
				   (uintmax_t)diff.tv_nsec);
...
main(void)
{
	...
	for (;;) {
		...
		if (module_vars.filename != NULL &&
			...
			log_module(module_vars.instance_name,
				   "The blob will be uploaded to: %s\n",
		...
		if (module_vars.upload != NULL &&
			...
			request.url = cb_data.blob_url = module_vars.upload;
			module_vars.upload = NULL;
			...
			result = EVP_blobOperation(h, EVP_BLOB_TYPE_HTTP,

...
static void
blob_get_upload_url_cb(EVP_BLOB_CALLBACK_REASON reason, const void *vp,
		       void *userData)
{
	const char *url = vp;
	check_expected(reason);
...
test_mstp_get_url(void **state)
{
	...
	// the blob operation should fire an mSTP token request
	result = EVP_blobGetUploadURL(ctxt->h, STORAGE_NAME, FILENAME,
				      blob_get_upload_url_cb, user_data);
	assert_int_equal(result, EVP_OK);

	// Expected next request
	check_request_and_respond(ctxt);

	// wait for the blob_get_upload_url_cb
	expect_value(blob_get_upload_url_cb, reason,
		     EVP_BLOB_CALLBACK_REASON_DONE);
	expect_string(blob_get_upload_url_cb, url, SOME_SAS_URL);
	expect_value(blob_get_upload_url_cb, userData, user_data);
...
main(void)
{
	...
	const struct CMUnitTest tests[] = {
		cmocka_unit_test(test_mstp),
		cmocka_unit_test(test_mstp_get_url),
	};

...
#define REQID_EVP1_TB "10004"
#define REQID_EVP2_TB "10004"

enum test_mstp_null_payloads { MSTP_REQ_TOPIC, MSTP_REQ, MSTP_RES };

static void *user_data = "some-user-data";

static const char mstp_request_topic_tb[] =
	"v1/devices/me/rpc/request/" REQID_EVP2_TB;
...
test_mstp_null_evp2(void)
{
	...
	agent_poll(verify_equals, agent_get_payload(MSTP_REQ_TOPIC));
	agent_poll(verify_equals, agent_get_payload(MSTP_REQ));

	agent_send_storagetoken_response(ctxt, agent_get_payload(MSTP_RES),
					 REQID_EVP1_TB);
...
setup(void **state)
{
	...
	agent_register_payload(MSTP_REQ_TOPIC, EVP_HUB_TYPE_EVP2_TB,
			       mstp_request_topic_tb);
